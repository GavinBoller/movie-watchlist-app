// @ts-nocheck
interface ClientFetchError extends Error {
  info?: any;
  status?: number;
  isOffline?: boolean;
  isTimeout?: boolean;
...
export const clientFetcher = async (
  ...
  if (typeof window !== 'undefined' && !window.navigator.onLine) {
    const offlineError: ClientFetchError = new Error('You are currently offline. Please check your internet connection.');
  ...
  let currentRetry = 0;
  let lastError: ClientFetchError | null = null;
    ...
    try {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          const error: ClientFetchError = new Error(errorMessage);
      ...
      if (error.name === 'AbortError') {
        lastError = new Error('Request timed out. Please try again.') as ClientFetchError;
        lastError.isTimeout = true;
      } else {
        lastError = error as ClientFetchError;
      }
