...
export default function MovieCard({ result, onAddToWatchlist, onInfoClick, isInWatchlist }: MovieCardProps) {
  const { 
    id, 
    title, 
    name, 
  ...
  const displayGenres = genres?.join(', ') || (genre_ids?.map(id => {
    const genresMap: GenreMap = media_type === 'tv' ? {
  ...
  const handleImdbClick = (e: MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
