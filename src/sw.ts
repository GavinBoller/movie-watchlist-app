...
sw.addEventListener('install', (event: ExtendableEvent) => {
  ...
  event.waitUntil(
    Promise.all([
      // Cache static assets
      caches.open(STATIC_CACHE_NAME).then((cache: Cache) => {
        console.log('Service Worker: Caching static assets');
        return cache.addAll(STATIC_ASSETS);
      }),
      // Cache API routes
      caches.open(API_CACHE_NAME).then((cache: Cache) => {
        console.log('Service Worker: Pre-caching API routes');
        // Pre-cache some API routes
        return Promise.allSettled(
          API_ROUTES.map((route: string) => 
            fetch(route)
              .then((response: Response) => response.ok ? cache.put(route, response) : null)
...
function isStaticAsset(pathname: string): boolean {
  return CACHEABLE_EXTENSIONS.some((ext: string) => pathname.endsWith(ext));
}
...
sw.addEventListener('message', (event: ExtendableMessageEvent) => {
  ...
  const { type, payload }: ServiceWorkerMessage = event.data;
...
// Error handling
sw.addEventListener('error', (event: ErrorEvent) => {
