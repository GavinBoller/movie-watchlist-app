...
// --- Configuration ---
// Path to your Prime Video viewing history CSV file
const PRIME_CSV_PATH = path.join(process.cwd(), 'prime-viewing-history.csv');
...
/**
 * Extracts the main title for search from a Prime Video viewing history entry.
 ...
 * @param {string} primeTitle - The title string from the Prime Video CSV.
 * @returns {string} The cleaned title suitable for TMDB search.
 */
function getSearchTitle(primeTitle) {
    let cleanedTitle = primeTitle.trim();
    ...
    const explicitEpisodePatterns = [
        ...
        /\s*-\s*Season\s+\d+.*$/i,                  // e.g., " - Season 2" (for Prime Video format)
        /\s*,\s*Season\s+\d+.*$/i,                  // e.g., ", Season 3"
        /\s+Season\s+\d+.*$/i,                      // e.g., " Season 1"
        /\s+S\d+.*$/i,                              // e.g., " S1", " S7"
        /\s*-\s*S\d+.*$/i,                          // e.g., " - S2"
        /\s*\(TV\)\s*-\s*SEASON\s+\d+.*$/i,         // e.g., "(TV) - SEASON 01"
...
async function searchTmdb(title) {
  ...
  //    - If the original Prime Video title doesn't contain common TV show indicators,
...
/**
 * Processes a single Prime Video viewing history item:
 * - Looks up the item on TMDB.
 * - Inserts or updates the item in the user's watchlist in the database.
 * @param {object} item - An object containing `primeTitle` and `watchedDate` from the CSV.
...
async function processWatchlistItem(item, client) {
  const searchTitle = getSearchTitle(item.primeTitle);
  const watchedAt = new Date(item.watchedDate);

  // Validate the date from the CSV
  if (isNaN(watchedAt.getTime())) {
    console.warn(`Skipping "${item.primeTitle}" - Invalid date format: "${item.watchedDate}"`);
    return false;
  }
  ...
  if (!tmdbInfo) {
    console.warn(`Could not find TMDB entry for search term "${searchTitle}" (original Prime Video title: "${item.primeTitle}")`);
    return false;
  }
    ...
    if (existingItem.rows.length > 0) {
      ...
      await client.query(
        ...
        [
            ...
            fullDetails.genres,
            'Prime Video' // Add Prime Video to the platform
        ]
      );
...
async function importPrimeHistory() {
  console.log('Starting Prime Video viewing history import process...');
  ...
  // 1. Read and parse the Prime Video CSV file
  console.log(`Reading CSV from: ${PRIME_CSV_PATH}`);
  const parser = fs.createReadStream(PRIME_CSV_PATH).pipe(
    parse({
  ...
  console.log(`Found ${records.length} entries in the Prime Video history CSV.`);

  // 2. Process each Prime Video history record with its own transaction
  for (const record of records) {
    ...
    try {
      ...
      console.error(`Critical error processing item "${record.primeTitle}": ${itemError.message}`);
  ...
  console.log('\nPrime Video viewing history import completed successfully!');
...
importPrimeHistory().catch(console.error);
