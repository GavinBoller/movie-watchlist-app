 ...
 * @param {string[]} options.preserveSlashesFor - Array of field names to preserve slashes for
 * @param {string} options.currentKey - Current field key being processed
 * @returns {any} Sanitized input
 */
export function sanitizeInput(input, options = {}) {
  const { preserveSlashesFor = ['poster', 'poster_path', 'image', 'url'], currentKey } = options;
  ...
  if (typeof input === 'string') {
    ...
    // Only encode slashes if this field is not in the preserve list
    if (!preserveSlashesFor.includes(currentKey)) {
    ...
    for (const [key, value] of Object.entries(input)) {
      sanitized[key] = sanitizeInput(value, { ...options, currentKey: key });
    }
    return sanitized;
  }
  return input;
